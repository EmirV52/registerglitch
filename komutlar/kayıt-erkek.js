...
exports.run = async (client, message, args) => {
 ...
 if(!['778979092246691891'].some(role => message.member.roles.cache.get(role)) && !message.member.hasPermission('ADMINISTRATOR')) return message.reply(`Bu Komut İçin Yetkiniz Bulunmamaktadır.`) 
  
let tag = "✧"
const kayıtlı = message.guild.roles.cache.find(r => r.id === '778979110038274059')
const kayıtsız = message.guild.roles.cache.find(r => r.id === '778979107168976916')
...
if(!member) return message.channel.send('Kayıt Edeceğin **Kullanıcıyı** Etiketlemen Gerekiyor.')
let stg = message.guild.member(member)
let isim = args[1]
let yas = args[2]
if(!isim) return message.reply('Bir **İsim** Belirtmen Gerekiyor.')
if(!yas) return message.reply('Bir **Yaş** Belirtmen Gerekiyor.')
...
.setTitle(`Kayıt İşlemi Tamamlandı`)
    ...
    .addField(`Yetkili Toplam:`, `\`${kayıtlar}\` Kayıta Sahip.`)
.setFooter(`Platon Registration`)
.setColor('GREEN')
client.channels.cache.get('778979166892589066').send(embed)
